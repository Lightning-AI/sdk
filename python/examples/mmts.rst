...
- Each machine writes its own artifacts; there is no combined folder for the
  run. ``mmt.download_artifacts`` gives each machine a folder named after it,
  and ``mmt.list_artifacts`` prefixes each path the same way. The jobs tutorial
  covers reading artifacts.
