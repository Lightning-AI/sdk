...
class SandboxesServiceApi(object):
    ...
    def sandboxes_service_extend_sandbox_timeout(self, body: 'SandboxesServiceExtendSandboxTimeoutBody', id: 'str', **kwargs) -> 'V1ExtendSandboxTimeoutResponse':  # noqa: E501
        """ExtendSandboxTimeout adds `timeout` more milliseconds to the sandbox's termination deadline. FailedPrecondition when the sandbox was created with an idle_timeout: its deadline belongs to KeepSandboxAlive and `timeout` is the cap that deadline is clamped to, so extending it here would push the sandbox past the cap it was created with. Also FailedPrecondition when a stop is already in progress.  # noqa: E501
    ...
    def sandboxes_service_keep_sandbox_alive(self, body: 'SandboxesServiceKeepSandboxAliveBody', id: 'str', **kwargs) -> 'V1KeepSandboxAliveResponse':  # noqa: E501
        """KeepSandboxAlive reports that a sandbox is doing work, pushing its idle deadline out to now + idle_timeout, clamped to the create-time cap. Callers that stop reporting let the sandbox expire through the normal timeout path. FailedPrecondition when the sandbox was created without an idle_timeout; retrying that can never work, so stop reporting for it. Aborted when a stop is already in progress and the keep-alive was not applied; retry on the next activity report.  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.sandboxes_service_keep_sandbox_alive(body, id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param SandboxesServiceKeepSandboxAliveBody body: (required)
        :param str id: (required)
        :return: V1KeepSandboxAliveResponse
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.sandboxes_service_keep_sandbox_alive_with_http_info(body, id, **kwargs)  # noqa: E501
        else:
            (data) = self.sandboxes_service_keep_sandbox_alive_with_http_info(body, id, **kwargs)  # noqa: E501
            return data

    def sandboxes_service_keep_sandbox_alive_with_http_info(self, body: 'SandboxesServiceKeepSandboxAliveBody', id: 'str', **kwargs) -> 'V1KeepSandboxAliveResponse':  # noqa: E501
        """KeepSandboxAlive reports that a sandbox is doing work, pushing its idle deadline out to now + idle_timeout, clamped to the create-time cap. Callers that stop reporting let the sandbox expire through the normal timeout path. FailedPrecondition when the sandbox was created without an idle_timeout; retrying that can never work, so stop reporting for it. Aborted when a stop is already in progress and the keep-alive was not applied; retry on the next activity report.  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.sandboxes_service_keep_sandbox_alive_with_http_info(body, id, async_req=True)
        >>> result = thread.get()
        ...
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method sandboxes_service_keep_sandbox_alive" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'body' is set
        if ('body' not in params or
                params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `sandboxes_service_keep_sandbox_alive`")  # noqa: E501
        # verify the required parameter 'id' is set
        if ('id' not in params or
                params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `sandboxes_service_keep_sandbox_alive`")  # noqa: E501
        ...
        return self.api_client.call_api(
            '/v1/core/sandboxes/{id}/keep-alive', 'POST',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='V1KeepSandboxAliveResponse',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
