...
def test_search_logs_maps_params(mocker_auth):
    job_api = JobApiV2()

    get_logs_mock = mock.MagicMock()
    job_api._client.jobs_service_get_logs = get_logs_mock

    job_api.search_logs(
        "ts-abc",
        job_ids=["job-1", "job-2"],
        sandbox_id="sbx-1",
        sandbox_command_ids=["cmd-1"],
        query="error",
        severity="warning",
        since="1h",
        page_size=50,
    ...
    kwargs = get_logs_mock.call_args.kwargs
    assert kwargs["project_id"] == "ts-abc"
    ...
    # page_size is sent as a string to the generated client
    assert kwargs["page_size"] == "50"
    assert kwargs["page_token"] == "tok-1"


def test_search_logs_omits_unset_params(mocker_auth):
    job_api = JobApiV2()

    get_logs_mock = mock.MagicMock()
    job_api._client.jobs_service_get_logs = get_logs_mock

    job_api.search_logs("ts-abc")
