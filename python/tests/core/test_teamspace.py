...
def test_download_dataset_version(
    ...
    # Mock list datasets response (empty - no matching dataset, dataset_id is a ULID)
    mock_list_response = mock.MagicMock()
    mock_list_response.data = json.dumps({"datasets": []})

    # Mock files response with one file
    ...
    try:
        with mock.patch.object(urllib.request, "urlretrieve", return_value=None) as mock_urlretrieve:
        ...
        mock_urlretrieve.assert_called_once_with(
            "https://presigned.example.com/data.csv", mock.ANY
        )
...
def test_download_dataset_version_no_token_no_cluster(
    ...
    # Mock list datasets response (empty - dataset_id is a ULID)
    mock_list_response = mock.MagicMock()
    mock_list_response.data = json.dumps({"datasets": []})
