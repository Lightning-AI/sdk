...
def _clear_lightning_env_vars(monkeypatch):
    ...
    A Studio exports real credentials and context (LIGHTNING_API_KEY, LIGHTNING_CLOUD_SPACE_ID,
    LIGHTNING_ORG, LIGHTNING_TEAMSPACE, ...). Tests are written assuming CI's environment, which
    sets none of that except one dummy value (see .github/workflows/unittest.yaml) -- just enough
    for ``Auth`` to short-circuit on ``_with_env_var`` without ever needing real credentials or a
    browser. Left ambient, the real vars leak the actual account into Auth() and the org/teamspace
    resolution fallbacks, letting API calls intended to hit a mock escape to the real backend
    instead; stripped entirely (including the one Auth relies on), a handful of tests that
    construct an API class without mocking Auth hang trying to open a browser for login. Tests
    that want a var set to something else can still do so themselves via ``monkeypatch.setenv``.
    """
    ...
    mocker.patch(
        ...
        return_value=V1ListDefaultClusterAcceleratorsResponse(
            accelerator=[
                ...
                V1ClusterAccelerator(
                    accelerator_type="GPU",
                    instance_id="rtx-6000-pro-1",
                    slug_multi_cloud="lit-rtx-6000-pro-1",
                    enabled=True,
                    resources=V1Resources(gpu=1),
                    family="RTXP_6000",
                ),
                V1ClusterAccelerator(
                    accelerator_type="GPU",
                    instance_id="rtx-6000-pro-4",
                    slug_multi_cloud="lit-rtx-6000-pro-4",
                    enabled=True,
                    resources=V1Resources(gpu=4),
                    family="RTXP_6000",
                ),
                V1ClusterAccelerator(
                    accelerator_type="GPU",
                    instance_id="rtx-6000-pro-8",
                    slug_multi_cloud="lit-rtx-6000-pro-8",
                    enabled=True,
                    resources=V1Resources(gpu=8),
                    family="RTXP_6000",
                ),
    ...
    mocker.patch(
        ...
        return_value=V1ListDefaultClusterAcceleratorsResponse(
            accelerator=[
                ...
                V1ClusterAccelerator(instance_id="rtx-6000-pro-1", slug_multi_cloud="lit-rtx-6000-pro-1", enabled=True),
                V1ClusterAccelerator(instance_id="rtx-6000-pro-4", slug_multi_cloud="lit-rtx-6000-pro-4", enabled=True),
                V1ClusterAccelerator(instance_id="rtx-6000-pro-8", slug_multi_cloud="lit-rtx-6000-pro-8", enabled=True),
