// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	stderrors "errors"
	"strconv"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// V1GetOrgModelAPIDailyLimitsResponse v1 get org model API daily limits response
//
// swagger:model v1GetOrgModelAPIDailyLimitsResponse
type V1GetOrgModelAPIDailyLimitsResponse struct {

	// Org-wide default cap in credits; unset means no default (members uncapped
	// unless individually overridden).
	DefaultDailyLimitCredits float64 `json:"defaultDailyLimitCredits,omitempty"`

	// member overrides
	MemberOverrides []*V1ModelAPIMemberDailyLimit `json:"memberOverrides"`
}

// Validate validates this v1 get org model API daily limits response
func (m *V1GetOrgModelAPIDailyLimitsResponse) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateMemberOverrides(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *V1GetOrgModelAPIDailyLimitsResponse) validateMemberOverrides(formats strfmt.Registry) error {
	if swag.IsZero(m.MemberOverrides) { // not required
		return nil
	}

	for i := 0; i < len(m.MemberOverrides); i++ {
		if swag.IsZero(m.MemberOverrides[i]) { // not required
			continue
		}

		if m.MemberOverrides[i] != nil {
			if err := m.MemberOverrides[i].Validate(formats); err != nil {
				ve := new(errors.Validation)
				if stderrors.As(err, &ve) {
					return ve.ValidateName("memberOverrides" + "." + strconv.Itoa(i))
				}
				ce := new(errors.CompositeError)
				if stderrors.As(err, &ce) {
					return ce.ValidateName("memberOverrides" + "." + strconv.Itoa(i))
				}

				return err
			}
		}

	}

	return nil
}

// ContextValidate validate this v1 get org model API daily limits response based on the context it is used
func (m *V1GetOrgModelAPIDailyLimitsResponse) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := m.contextValidateMemberOverrides(ctx, formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *V1GetOrgModelAPIDailyLimitsResponse) contextValidateMemberOverrides(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.MemberOverrides); i++ {

		if m.MemberOverrides[i] != nil {

			if swag.IsZero(m.MemberOverrides[i]) { // not required
				return nil
			}

			if err := m.MemberOverrides[i].ContextValidate(ctx, formats); err != nil {
				ve := new(errors.Validation)
				if stderrors.As(err, &ve) {
					return ve.ValidateName("memberOverrides" + "." + strconv.Itoa(i))
				}
				ce := new(errors.CompositeError)
				if stderrors.As(err, &ce) {
					return ce.ValidateName("memberOverrides" + "." + strconv.Itoa(i))
				}

				return err
			}
		}

	}

	return nil
}

// MarshalBinary interface implementation
func (m *V1GetOrgModelAPIDailyLimitsResponse) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *V1GetOrgModelAPIDailyLimitsResponse) UnmarshalBinary(b []byte) error {
	var res V1GetOrgModelAPIDailyLimitsResponse
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
