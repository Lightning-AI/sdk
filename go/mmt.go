...
type MachineDict struct {
	// Name is the machine job name.
	Name string `json:"name"`
	// Status is the latest known machine job state.
	Status string `json:"status"`
	// Machine is the assigned machine type.
	Machine string `json:"machine"`
}

// MMTDict is the JSON-friendly public representation of an MMT.
type MMTDict struct {
	// Name is the multi-machine job display name.
	Name string `json:"name"`
	// Teamspace is the owning teamspace name.
	Teamspace string `json:"teamspace"`
	// Studio is the source studio name when the MMT was launched from a studio.
	Studio *string `json:"studio"`
	// Image is the container image used by the MMT.
	Image *string `json:"image"`
	// Command is the shell command run by each machine.
	Command string `json:"command"`
	// Status is the latest known MMT state.
	Status string `json:"status"`
	// Machine is the machine type assigned to each MMT worker.
	Machine string `json:"machine"`
	// Machines lists the worker jobs that belong to the MMT.
	Machines []MachineDict `json:"machines"`
	// TotalCost is the accumulated MMT cost reported by Lightning.
	TotalCost float32 `json:"total_cost"`
}

// MMTPathMapping maps a teamspace connection path into an MMT container.
type MMTPathMapping struct {
	// ContainerPath is the mount path inside each worker container.
	ContainerPath string
	// ConnectionName is the teamspace connection to mount.
	ConnectionName string
	// ConnectionPath is the path within the teamspace connection.
...
type MMTOptions struct {
	// ID resolves an existing MMT directly without looking it up by name.
	ID string
	// Teamspace scopes MMT lookup or creation to a specific teamspace.
	Teamspace *Teamspace
	// Studio launches the MMT from a specific studio environment.
	Studio *Studio
	// Status seeds the latest known state for ID-based handles.
	Status string
	// NumMachines is the number of worker machines to start.
	NumMachines int64
	// Machine selects the compute type used for each worker.
	Machine Machine
	// Command is the command executed by each worker.
	Command string
	// Image is the container image used when creating an MMT.
	Image string
	// TotalCost seeds the latest known cost for ID-based handles.
	TotalCost float32
	// Env sets environment variables for each worker.
	Env map[string]string
	// Cloud selects a cloud account or provider for the MMT.
	Cloud string
	// Interruptible overrides whether workers run on interruptible capacity.
	Interruptible *bool
	// ImageCredentials names credentials used to pull the container image.
	ImageCredentials string
	// CloudAccountAuth enables cloud account authentication for workers.
	CloudAccountAuth bool
	// Entrypoint overrides the container entrypoint.
	Entrypoint string
	// PathMappings mounts teamspace connection paths into each worker container.
	PathMappings []MMTPathMapping
	// ArtifactsSource is the local path copied from workers after completion.
	ArtifactsSource string
	// ArtifactsDestination is the remote teamspace connection destination.
...
type MMTWaitOptions struct {
	// Interval controls how often MMT status is polled.
	Interval time.Duration
	// Timeout limits how long Wait polls before returning.
	Timeout time.Duration
	// StopOnTimeout stops the remote MMT when the timeout expires.
	StopOnTimeout bool
}
