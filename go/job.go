...
type JobDict struct {
	...
	// Studio is the source studio name when the job was launched from a studio.
	Studio *string `json:"studio"`
	// Image is the container image used by the job.
	Image *string `json:"image"`
	// Command is the shell command run by the job.
	Command string `json:"command"`
	// Status is the latest known job state.
	Status string `json:"status"`
	// Machine is the machine type assigned to the job.
	Machine string `json:"machine"`
	// TotalCost is the accumulated job cost reported by Lightning.
	TotalCost float32 `json:"total_cost"`
}

// JobPathMapping maps a teamspace connection path into a job container.
type JobPathMapping struct {
	// ContainerPath is the mount path inside the job container.
...
type ScratchDisk struct {
	// Path is the mount path inside the job container.
	Path string
	// SizeGB is the scratch disk size in gigabytes.
	SizeGB int
}
...
type JobOptions struct {
	// ID resolves an existing job directly without looking it up by name.
	ID string
	// Teamspace scopes job lookup or creation to a specific teamspace.
	Teamspace *Teamspace
	// Studio launches the job from a specific studio environment.
	Studio *Studio
	// MMT associates the job with a parent multi-machine job.
	MMT *MMT
	// Status seeds the latest known state for ID-based handles.
	Status string
	// Machine selects the compute type used when creating a job.
	Machine Machine
	// Command is the command executed by the job.
	Command string
	// Image is the container image used when creating a job.
	Image string
	// PublicIP seeds the latest known public IP for ID-based handles.
	PublicIP string
	// TotalCost seeds the latest known cost for ID-based handles.
	TotalCost float32
	// Env sets environment variables for the job.
	Env map[string]string
	// Cloud selects a cloud account or provider for the job.
	Cloud string
	// Interruptible overrides whether the job runs on interruptible capacity.
	Interruptible *bool
	// ImageCredentials names credentials used to pull the container image.
	ImageCredentials string
	// CloudAccountAuth enables cloud account authentication in the job.
	CloudAccountAuth bool
	// Entrypoint overrides the container entrypoint.
	Entrypoint string
	// PathMappings mounts teamspace connection paths into the job container.
	PathMappings []JobPathMapping
	// ArtifactsSource is the local path copied from the job after completion.
	ArtifactsSource string
...
type JobWaitOptions struct {
	// Interval controls how often job status is polled.
	Interval time.Duration
	// Timeout limits how long Wait polls before returning.
	Timeout time.Duration
	// StopOnTimeout stops the remote job when the timeout expires.
	StopOnTimeout bool
}
