...
@pytest.mark.parametrize("studio_id, expect_error, error_message, expect_info", [
    ("st-abc", False, "", ""),
    ("st-def", True, "abc", ""),
    ("st-ghi", True, "", ""),
    ("st-jkl", True, "jkl", ""),
...
@pytest.mark.parametrize("studio_id, expect_error, error_message", [
    ("st-abc", False, ""),
    ("st-def", True, "abc"),
    ("st-ghi", True, ""),
    ("st-jkl", True, "jkl"),
...
@pytest.mark.parametrize("studio_id, expect_error, error_message, expected_port", [
    ("st-abc", False, "", 0),
    ("st-def", False, "", 1),
    ("st-ghi", False, "", -1),
...
def test_execute_plugin(internal_studio_api_execute_plugin_mocker, studio_id, expect_error, error_message, expected_port):
    studio_api = StudioApi()
    ...
    if not expect_error:
        ...
        if port > 0:
            assert output_str == f"Plugin my-fancy-plugin is interactive. Have a look at https://{expected_port}-{studio_id}.cloudspaces.litng.ai"
        elif port == 0:
...
def test_create_mmt(internal_studio_api_create_app_mocker):
    ...
    resp = studio_api.create_multi_machine_job("my-entry-point", "fancy-mmt-name", 4, Machine.A10G, "parallel", "st-abc", "ts-abc", "cluster-abc")
    assert resp.name == "fancy-mmt-name"

def test_create_inference_run(internal_studio_api_create_app_mocker):
    studio_api = StudioApi()

    resp = studio_api.create_inference_job("my-entry-point", "fancy-inference-name", Machine.A10G, min_replicas='1', max_replicas='5', max_batch_size='10', timeout_batching='0.3', scale_in_interval='11', scale_out_interval='12', endpoint="/fancy-predict", studio_id="st-abc", teamspace_id="ts-abc", cluster_id="cluster-abc")
