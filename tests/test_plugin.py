...
def test_run_job(internal_studio_init_mocker, internal_studio_status_mocker, internal_job_run_mocker, cloud_compute):
    studio = Studio("st-ghi", "ts-abc", "org-abc")
    plugin = JobsPlugin("jobs", "Launch asynchronous scripts from a Studio - Like submitting a job to a cluster", studio)
...
def test_run_mmt(internal_studio_init_mocker, internal_studio_status_mocker, internal_mmt_run_mocker, cloud_compute):
    studio = Studio("st-ghi", "ts-abc", "org-abc")
    plugin = MultiMachineTrainingPlugin("multi-machine-training", "Train a model across multiple cloud machines", studio)
...
def test_run_inference(internal_studio_init_mocker, internal_studio_status_mocker, internal_inference_run_mocker, cloud_compute):
    ...
    plugin.run(command="python my-file.py", name="my-fancy-inference-name", min_replicas=1, max_replicas=5, max_batch_size=10, timeout_batching=0.3, scale_in_interval=11, scale_out_interval=12, endpoint="/fancy-predict", cloud_compute=cloud_compute)
