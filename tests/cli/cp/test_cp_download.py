...
def test_route_cp_s3_folders_download():
    ...
    with patch("lightning_sdk.cli.cp.Filesystem", return_value=mock_fs):
        route_cp_operation(
            source="lit://my-org/my-teamspace/s3_folders/data/model.ckpt",
            destination="/local/model.ckpt",
            recursive=False,
        )
...
def test_route_cp_s3_folders_download_recursive():
    """Test that s3_folders download passes recursive=True to Filesystem.copy."""
    mock_fs = MagicMock()

    with patch("lightning_sdk.cli.cp.Filesystem", return_value=mock_fs):
        route_cp_operation(
            source="lit://my-org/my-teamspace/s3_folders/data/mydir",
...
def test_route_cp_s3_folders_raises_if_both_remote():
    """Test that two lit:// paths raises ValueError."""
    with pytest.raises(ValueError, match="two remote URLs"):
        route_cp_operation(
            source="lit://my-org/my-teamspace/s3_folders/a.txt",
            destination="lit://my-org/my-teamspace/s3_folders/b.txt",
        )


def test_route_cp_s3_folders_raises_if_both_local():
